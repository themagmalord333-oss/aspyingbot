        ...
        async with aiohttp.ClientSession(headers=get_headers()) as session:
            async with session.get(url) as resp:
                if resp.status == 200:
                    ...
                    print(f"TonAPI Error: Status {resp.status} on endpoint {endpoint}")
    except Exception as e:
        print(f"TonAPI Network Error: {e}")
    return None
...
async def fetch_wallet_nfts(address: str) -> dict:
    ...
    if not data: 
        return {"error": "Failed to fetch NFTs from TonAPI."}
...
async def fetch_wallet_stats(address: str) -> dict:
    ...
    if not data: 
        return {"error": "Could not fetch transaction stats."}
