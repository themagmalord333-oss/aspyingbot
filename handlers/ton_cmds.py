...
async def cmd_balance(message: Message):
    ...
    if len(args) < 2: 
        return await message.answer("Usage: `/balance <address>`", parse_mode="Markdown")
...
async def cmd_nft(message: Message):
    ...
    if "error" in data: 
        return await msg.edit_text("❌ Failed to load portfolio. Check TON_API_KEY.")
...
async def cmd_whales(message: Message):
    text = (
        "🐋 **Top Whales (Fragment Collection):**\n\n"
        "1. `EQCA14o1-VWhS2efVKHN...`\n"
        "2. `EQBvW8Z5huPt351jIG7F...`\n"
        "*(Live blockchain indexing required for full global list)*\n\n"
        "🛡 *Powered by Anysnap*"
    )
