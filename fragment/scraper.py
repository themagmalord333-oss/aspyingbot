...
async def fetch_history(username: str) -> list:
    ...
    for row in rows:
        ...
        if date and price:
            history.append(f"{date.text.strip()} - {price.text.strip()} TON")
    return history[:5]
...
async def fetch_pricing(endpoint: str):
    soup = await fetch_soup(f"{BASE_URL}/{endpoint}")
    if not soup: return "Unknown"
    price = soup.find("div", class_="tm-min-btn-value icon-before icon-ton")
